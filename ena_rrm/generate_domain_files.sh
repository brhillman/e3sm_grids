...
# Find conservative mapping files, use latest file generated (ocn -> atm)
...
# Find conservative mapping files, use latest file generated (ocn -> lnd)
map_ocn_to_target=`ls ${output_root}/map_${ocn_grid_name}_to_${lnd_grid_name}_mono.*.nc | tail -n1`

# Run gen_domain
${gen_domain} -m ${map_ocn_to_target} -o ${ocn_grid_name} -l ${lnd_grid_name}
