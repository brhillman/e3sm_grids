...
# Edit SBATCH directives, machine, resolution for Cori
cd ${e3sm_root}/components/homme/test/template/
sed -i s/#SBATCH --account=FY127788/#SBATCH --account=acm/
sed -i s/ /#SBATCH --partition=regular
sed -i s/ /#SBATCH --constraint=knl
sed
